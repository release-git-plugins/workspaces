...
published, in order to configure `release-git` to avoid attempting to publish
the top level package (in addition to publishing your workspace packages), you
would add the following to your `release-git` config (again showing
...
`@release-git-plugins/workspaces` publishes to the `npm` registry.
