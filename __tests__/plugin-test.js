...
const namespace = '@release-git-plugins/workspaces';
...
function buildPlugin(config = {}, _Plugin = TestPlugin) {
  ...
  plugin.spinner.show = (options) => {
    ...
    if (options.prompt) {
      ...
      // uses the same prompting logic from release-git itself:
      //
      // https://github.com/release-git/release-git/blob/13.5.0/lib/prompt.js#L19-L24
  ...
  // this works around a fairly fundamental issue in release-git's testing
...
describe('@release-git-plugins/workspaces', () => {
